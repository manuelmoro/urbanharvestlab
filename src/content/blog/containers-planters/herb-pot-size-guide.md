---
title: "Herb Pot Size Guide: Basil, Parsley, Mint, Thyme, and Chives"
description: "Choosing the right pot size is critical for indoor herbs. Learn how big containers need to be for basil, parsley, mint, thyme, and chives."
excerpt: "Stop suffocating your herbs in tiny decorative cups. This guide breaks down the exact container sizes needed for the most common indoor and balcony herbs."
slug: "containers-planters/herb-pot-size-guide"
category: "containers-planters"
tags:
  - herbs
  - containers
  - pot size
  - beginner
  - urban gardening
published: true
draft: false
cluster_role: "supporting"
affiliate: false
featured: false
popular: false
publishDate: 2026-03-15
image: /images/containers-planters/herb-pot-size-guide.webp
imageAlt: Clean UI illustration showing different sized terracotta pots for Basil, Parsley, and Thyme
keyTakeaways:
  - Basil requires the largest, deepest pot you can provide to support its heavy water consumption, especially on a sunny balcony.
  - Mediterranean herbs like thyme prefer wider, shallower pots that dry out quickly, mimicking their rocky native soils.
  - Slower-growing herbs like chives and parsley do well in medium pots, provided they have adequate drainage.
beginnerPathTitle: Build your container system in this order
beginnerPathSlugs:
  - urban-gardening/small-space-herb-gardening
  - containers-planters/best-containers-herbs
  - urban-gardening/best-herbs-small-spaces
topicLinksTitle: Read these if you are selecting herbs and pots
topicLinksDescription: These guides will help you match the right herb to the right sized container based on your available sunlight.
topicLinkSlugs:
  - urban-gardening/small-space-herb-gardening
  - containers-planters/best-containers-herbs
  - urban-gardening/best-herbs-small-spaces
  - urban-gardening/best-herbs-sunny-balcony
faq:
  - question: Why is my basil struggling in a 4-inch pot?
    answer: Basil has a massive root system and drinks a lot of water. A 4-inch pot dries out far too quickly to support a mature basil plant, leading to daily wilting and stress.
  - question: Can I plant all my herbs in one large planter?
    answer: It is usually a bad idea. Herbs have drastically different water and root space needs. Separate pots allow you to control the environment for each specific plant.
  - question: Does thyme need a deep pot?
    answer: No. Thyme has shallow roots and spreads horizontally. A deep pot will hold moisture at the bottom where the roots cannot reach, leading to heavy, waterlogged soil.
comparisonTable:
  caption: "Ideal container dimensions for common culinary herbs"
  columns:
    - Herb
    - Minimum Pot Diameter
    - Ideal Pot Shape
    - Root Depth
  rows:
    - ["Basil", "10-12 inches", "Large, deep bucket", "Deep and aggressive"]
    - ["Parsley", "8-10 inches", "Standard medium depth", "Deep taproot"]
    - ["Mint", "10-12 inches", "Wide and self-contained", "Spreading runners"]
    - ["Thyme", "6-8 inches", "Wide and shallow", "Shallow and creeping"]
    - ["Chives", "6-8 inches", "Standard round or square", "Shallow bulbs"]
relatedSlugs:
  - urban-gardening/small-space-herb-gardening
  - containers-planters/best-containers-herbs
  - urban-gardening/best-herbs-small-spaces
  - urban-gardening/best-herbs-sunny-balcony
---

The quickest way to fail at apartment herb gardening is to fall for the "tiny pot" aesthetic. Social media is filled with images of enormous, lush basil plants thriving in minuscule teacups. Those photos are staged. In reality, a mature herb confined to an undersized pot will dry out rapidly, become root-bound, and quickly die.

Before you buy soil or seeds, you need to understand the container rules outlined in [Best Containers for Herbs: Pot Size, Drainage, and Material Guide](/blog/containers-planters/best-containers-herbs/). That guide establishes *how* drainage and materials work. This current guide explains exactly *what size* container you need for the five most common indoor herbs.

If you are just getting started organizing your space, remember to zoom out and read [Small-Space Herb Gardening: Start Here for Apartments, Balconies, and Indoor Setups](/blog/urban-gardening/small-space-herb-gardening/) to see where these containers fit into your broader plan.

Here is the sizing breakdown for the herbs you are most likely trying to grow.

## 1. Basil: The Giant Thirst

Basil is a big, fleshy plant that grows rapidly when exposed to heat and light. It behaves more like a hungry vegetable than a delicate herb.

- **The Root System:** Basil roots dive deep and spread fast. If they hit the walls of a small pot, growth stalls instantly.
- **The Water Problem:** Basil leaves transpire (sweat) incredibly fast. In a small pot on a hot day (or under a strong grow light), the soil will dry in a matter of hours. This causes the dramatic wilting you often see by mid-afternoon.
- **The Ideal Pot:** Go as large as you can reasonably fit in your space. A 10-inch to 12-inch diameter pot that is equally deep is the bare minimum for a healthy summer basil plant. Larger is better. If you are growing it outside, the rules in [Best Herbs for a Sunny Balcony: Basil, Thyme, Rosemary, and Other Reliable Choices](/blog/urban-gardening/best-herbs-sunny-balcony/) explain exactly why the largest pot is non-negotiable for sun exposure.

## 2. Thyme: The Shallow Creeper

Thyme comes from rocky, dry, Mediterranean environments. It does not grow a deep central taproot; instead, it creeps across the surface, putting down fine, shallow roots as it spreads.

- **The Problem with Deep Pots:** Deep pots are terrible for thyme. The bottom half of a deep pot stays wet long after the top has dried out. Because thyme's roots never reach the bottom, that wet soil just stagnates and breeds rot.
- **The Ideal Pot:** Thyme wants a pot that is wider than it is deep. Shallow bowls, window boxes, or wide terracotta pans are perfect. A 6-inch to 8-inch width is enough to start, provided the depth is kept to a minimum (4 to 6 inches).

## 3. Parsley: The Deep Taproot

Unlike thyme, parsley relies on a long central taproot (similar to a carrot) to anchor itself.

- **The Root Constraint:** If you put parsley in a shallow bowl, the taproot hits the bottom, curves, and the plant becomes stunted. It will never send up the thick, robust leaves you want for cooking.
- **The Ideal Pot:** Parsley needs depth, not width. A standard, medium-sized pot (8 to 10 inches tall and wide) works perfectly. The footprint on your shelf can be small, as long as the vertical drop is there.

## 4. Mint: The Aggressive Runner

Mint is an incredible survivor, which is exactly the problem. Under the soil, mint sends out aggressive runners (rhizomes) that shoot across the pot to sprout new stems.

- **The Mixed Container Mistake:** Never plant mint in a large shared container with other herbs. Given a few weeks, its runners will cross the pot, strangle the other plants, and take over the entire volume of soil.
- **The Ideal Pot:** Mint must be kept in solitary confinement. Give it a medium-to-large container (10 to 12 inches wide) so it can bush out and produce heavy harvests, but keep it strictly alone. Width is slightly more important than extreme depth here.

## 5. Chives: The Tight Clump

Chives grow from tiny, shallow bulbs that multiply in tight clusters. They don't have deep roots, and they don't send out invasive runners like mint.

- **The Space Requirement:** Chives are highly forgiving when it comes to space. They will happily fill whatever container you put them in, eventually forming a dense, grassy clump.
- **The Ideal Pot:** A standard 6-inch or 8-inch pot is perfect. You don't need excessive depth or extreme width. They are the easiest herb to fit into an awkward or tight spot on your shelf, which is why they are often recommended in [Best Herbs for Small Spaces: What Grows Well in Apartments, Balconies, and Windowsills](/blog/urban-gardening/best-herbs-small-spaces/).

## Why Pups and Seedlings Can't Stay in Nursery Pots

When you buy a herb from the supermarket or garden center, it usually comes in a flimsy 3-to-4 inch square plastic nursery pot. The golden rule of container gardening is that *these pots are temporary holding cells, not permanent homes.* 

By the time a herb makes it to the supermarket shelf, its roots have already filled that tiny plastic cup. If you leave it there, it has no room to expand, no excess soil to hold water, and no buffer against temperature swings. As soon as you bring an herb home, map it to the sizes above and immediately repot it.
